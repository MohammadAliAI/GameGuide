...
class Recommender:
    def __init__(self, values, user_idx, product_idx):
    ...
    def create_and_fit(self, model_name: str, weight_strategy: str = "bm25", model_params: Dict[str, Any] = {}):
        weight_strategy = weight_strategy.lower()
        if weight_strategy == "bm25":
            data = bm25_weight(self.user_product_matrix, K1=1.2, B=0.75)
    ...
    def recommend_products(self, user_id, items_to_recommend=5):
        if not self.fitted:
            raise InternalStatusError("Cannot recommend products without fitting the model.")
    ...
    def explain_recommendation(self, user_id, suggested_item_id, recommended_items):
        _, items_score_contrib, _ = self.model.explain(user_id, self.user_product_matrix, suggested_item_id, N=recommended_items)
